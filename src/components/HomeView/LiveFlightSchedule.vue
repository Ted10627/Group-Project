...
const flightLink = computed(() => {
  return category.value === 'international' ? '/InternationalFlights' : '/DomesticFlights'
})
...
<template>
  ...
  >
    <div class="flex">
      ...
      <!-- 下面無功能，為了版面而占用空間 -->
      <div class="relative p-5 w-[500px] h-[110px] items-center hidden lg:block">
      ...
      <table class="w-full">
            ...
            <table class="w-full">
              <thead class="h-[65px] bg-[#343557]">
                <tr class="airportAuxiliaryColor-2">
                  ...
                  <th v-if="displayType.includes('Arrival')" class="text-center lg:w-[25%]">
                    <div class="flex justify-center items-center border-r-2">出發地</div>
                  </th>
                  <th v-if="displayType.includes('Departure')" class="text-center lg:w-[25%]">
        ...
        <tbody class="bg-white">
          <tr v-if="filterFlights.length === 0">
            <td colspan="5" class="text-center md:subtitle py-4 bg-slate-200">查無資料</td>
          </tr>
          <tr
